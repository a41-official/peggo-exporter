...
class PeggoMetrics:
    ...
    def __init__(self, api_url, orchestrator_address, polling_interval_seconds):
        ...
        self.last_orchestrator_balance = 0

        # Prometheus metrics to collect
        self.last_observed_nonce = Gauge("peggo_last_observed_nonce", "Network Current Observed Peggo Nonce")
        ...
        self.orchestrator_balance = Gauge("peggo_orchestrator_balance", "Peggo Orchestrator Balance")
    ...
    def fetch(self):
        ...
        self.orchestrator_balance.set(self.get_orchestrator_balance(self.api_url))
    ...
    def get_orchestrator_balance(self, api_url):
        """
        Get Orchestrator Balance
        """

        # Build URL
        orchestrator_balance_url = api_url + "/cosmos/bank/v1beta1/balances/" + self.orchestrator_address + "/by_denom?denom=inj"
        logger.debug('Get Orchestrator Balance: API Request: ')
        logger.debug(orchestrator_balance_url)

        # Make API call to get orchestrator balance
        response = self.submit_api_request(orchestrator_balance_url)

        # Exception encountered in connectivity, return last orchestrator balance
        # Error already logged in submit_api_request
        if response is  None:
            return self.last_orchestrator_balance
        
        orchestrator_balance = response.json()
        logger.debug('Get Orchestrator Balance: API Response: orchestrator_balance: ')
        logger.debug(orchestrator_balance)

        # Check for error response
        self.check_for_error(orchestrator_balance)

        # Get orchestrator balance from API response
        self.last_orchestrator_balance = float(orchestrator_balance['balance']['amount']) / 10**18
        logger.info('Orchestrator Balance: ' + str(float(orchestrator_balance['balance']['amount']) / 10**18))

        # Return Orchestrator Balance
        return (float(orchestrator_balance['balance']['amount']) / 10**18)
